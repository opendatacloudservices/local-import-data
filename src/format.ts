...
export const standardizeFormat = (format: string, url: string): string => {
  if (format) {
    // clean string
    ...
    if (
      format.indexOf('gml application schema') >= 0 ||
  ...
  if (url) {
    // overrules all
    const urlFormat = mimeService(url);
    if (urlFormat) {
      format = urlFormat;
    }
  }
...
export const mimeService = (url: string | null): string | null => {
  let re = null;
  if (!url) {
    return null;
  }
