...
class CSW extends index_1.Harvester {
    ...
    async import(next) {
        ...
        for (let i = 0; i < next.length; i += 1) {
            try {
                const exists = await this.exists('csw', next[i].instance, next[i].dataset);
                if (exists) {
    ...
    getDataset(instance, dataset) {
        return this.getPrefix(instance).then(prefix => {
            ...
            return Promise.all(queries.map(query => {
                ...
                if (results[0].rows[0].spatial_description &&
                    Array.isArray(returnObject.spatialDescription)) {
                    returnObject.spatialDescription.push(...results[0].rows[0].spatial_description);
                }
                if (results[0].rows[0].geographic_description &&
                    Array.isArray(returnObject.spatialDescription)) {
                    returnObject.spatialDescription.push(...results[0].rows[0].geographic_description);
                }
                ...
                const created = [new Date(results[0].rows[0].date_stamp)];
                const modified = [new Date(results[0].rows[0].date_stamp)];
                ...
                returnObject.modified = moment(created[0]).format('YYYY-MM-DD HH:mm:ss');
